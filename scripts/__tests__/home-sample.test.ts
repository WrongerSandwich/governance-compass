...
function tensionProfile(
  personaId: string,
  model: string,
  tensions: TensionProfile["tensions"],
): TensionProfile {
  return { persona_id: personaId, model, tensions };
}
...
describe("selectHomeSampleTension", () => {
  it("rejects an axis that only one model administration marks strong", () => {
    const profiles = [
      tensionProfile("A", "claude", [
        { axis: 2, magnitude: 1.8, level: "strong" },
      ]),
      tensionProfile("A", "gemini", [
        { axis: 2, magnitude: 1.4, level: "moderate" },
      ]),
      tensionProfile("B", "claude", []),
      tensionProfile("B", "gemini", []),
    ];

    expect(selectHomeSampleTension(profiles, "A", "B")).toBeNull();
  });

  it("ranks mutually strong axes by mean magnitude and keeps respondent A first", () => {
    const profiles = [
      tensionProfile("A", "claude", [
        { axis: 1, magnitude: 1.9, level: "strong" },
        { axis: 2, magnitude: 1.7, level: "strong" },
      ]),
      tensionProfile("A", "gemini", [
        { axis: 1, magnitude: 1.1, level: "strong" },
        { axis: 2, magnitude: 1.5, level: "strong" },
      ]),
      tensionProfile("B", "claude", [
        { axis: 9, magnitude: 2, level: "strong" },
      ]),
      tensionProfile("B", "gemini", [
        { axis: 9, magnitude: 2, level: "strong" },
      ]),
    ...
    expect(selectHomeSampleTension(profiles, "A", "B")).toEqual({
      axis: 2,
      respondent: "a",
    });
  });

  it("uses the lower axis id to break equal-mean ties", () => {
    const profiles = [
      tensionProfile("A", "claude", [
        { axis: 4, magnitude: 1.7, level: "strong" },
        { axis: 7, magnitude: 1.5, level: "strong" },
      ]),
      tensionProfile("A", "gemini", [
        { axis: 4, magnitude: 1.3, level: "strong" },
        { axis: 7, magnitude: 1.5, level: "strong" },
      ]),
    ];
  ...
  it("falls back to respondent B and records its provenance", () => {
    const profiles = [
      tensionProfile("A", "claude", [
        { axis: 3, magnitude: 1.8, level: "strong" },
      ]),
      tensionProfile("A", "gemini", []),
      tensionProfile("B", "claude", [
        { axis: 7, magnitude: 1.6, level: "strong" },
      ]),
      tensionProfile("B", "gemini", [
        { axis: 7, magnitude: 1.4, level: "strong" },
      ]),
    ];
...
describe("home_sample_pair.json (committed artifact)", () => {
  ...
  it("records which respondent supplied the consensus tension", () => {
    expect(artifact.tension_respondent).toBe("a");
  });
