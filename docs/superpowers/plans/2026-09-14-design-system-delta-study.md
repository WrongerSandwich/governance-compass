...
**D28 — `WorldMap`'s `outline: "none"` is an accessibility defect and is removed.** `WorldMap.tsx:580` and `:627` render region paths with `tabIndex={0} role="button"`, and their `default`, `hover` and `pressed` style objects each set `outline: "none"`. react-simple-maps has no `focus` entry in that style object, so there is nothing to restore the ring. **An earlier draft of this decision said a keyboard user therefore got no focus indicator at all. That is false**, and Task 11's reviewer caught it: `WorldMap.tsx:140-143` maps `onFocus` onto the hover state, so a keyboard user gets the `hover` entry — a region-shaped highlight. The accurate statement is that they get **a cue indistinguishable from mouse hover**, which is not a conforming focus indicator (WCAG 2.4.7 wants focus to be perceivable *as focus*). The fix is unchanged and is still worth making; only the severity claim was wrong. The lesson is the cheaper one to learn twice: before writing "there is no indicator", tab to the element and look. This is the same defect class as the `focus:outline-none` spelling phase 1 removed from 25 call sites, arriving by a different route: there, the ring was declared and never painted; here it is painted over. The `outline: "none"` entries go from the two interactive branches and `focus-ring` goes on via `className`. The non-interactive branches (`tabIndex={-1}`, `pointerEvents: "none"`) keep theirs — an unfocusable path has no ring to suppress, and removing it there would be churn.

**D29 — `PersonaModal`'s local `ScoreBar` converges on `PairedAxisScale`.** `PersonaModal.tsx:612` defines a private `ScoreBar` whose mapping is `((score + 1) / 2) * 100` — the pre-delta spelling that phase 4 replaced site-wide with `scoreToTrackPercent`'s `50 + score * 44`, and whose alignment bug phase 4 fixed in `GroupScoreBar` (a dot at a pole clips the track end, and the midline no longer lands where the dots say 0 is). It is also, by name, a component the repo retired: phase 5 Task 5 deleted `ScoreBar.tsx` and left an import guard behind, which this local declaration passes because it is not an import. The modal's Claude-vs-Gemini bar is **not** `PairedAxisScale`'s two-respondent variant, though an earlier draft of this decision said it was. The dual row is two independent `flex: 1` model columns — each with its own track, `mono-meta` readout, confidence dot and tension badge — plus a `flex: 0 0 38px` delta column, aligned under the CLAUDE / GEMINI / Δ header spans. The two-respondent variant is one track with two dots; collapsing to it would destroy the per-model readouts and the delta column's alignment, and would colour B's dot outlined Stone 500, erasing the model distinction `markVar` exists to preserve. **Task 9 mandates the single-respondent variant at all five sites and is the authoritative reading.** Task 9's implementer flagged the contradiction rather than silently picking one, and both reviewers confirmed the correction against the live DOM. The convergence is still a fit rather than a squeeze, and it brings the modal a generated `aria-label` where it currently has `role="presentation"` and nothing else.
...
**One non-type site routed here from Task 7, because it is one decision across two files — and the routing note's own conclusion is overruled.** `PersonaModal.tsx:137` and `CompareView.tsx:38` each declare the mini budget strip's seven ministry fills as six `--cluster-*` tokens plus a seventh spelled `var(--stone-400)`, which Task 14's section-wide guard bans. Task 7 correctly declined to swap it and showed, token by token, that every mode-stepping value `globals.css` declares collides with one of the six fills beside it. It then concluded: mint a seventh. **Do not.** Spec "Scope and constraints" opens with *"No new colours. Every hex in the handoff already exists,"* and a categorical ramp grown by one to satisfy a guard is exactly the kind of quiet scope creep that rule exists to stop.

The premise the collision analysis never tested is the one that resolves it: **`--stone-400` is mode-invariant.** It is declared once in `:root` and never redefined in the dark block — `#b5a594` in both modes, a mid tone that reads on `#ffffff` and on `#2a2118` alike. The guard bans 600 and 400 because those are the **mark** tones and a *mark* must step in sympathy with every other mark on the page. This is not a mark. It is one categorical fill among seven, and its job is only to differ from the six beside it — the same reasoning that already exempts `--stone-900` and `--stone-50` as contrast ink.

So: **keep the value and exempt it, narrowly.** The two arrays are byte-identical and one of them (`PersonaModal`'s, named `COLORS`) is declared *inside* the component and reallocated on every render, so the exemption comes free with a real fix — extract them to a single `src/lib/study/budgetColors.ts` exporting `BUDGET_COLORS`, point both files at it, and add that one filename to Task 14's guard exclusion with the reasoning above written into the comment. Grow the exclusion list, do not weaken the guard: that is the shape phase 5 used for `SVG_CAPPED`, and it keeps the ban live on all ~47 other files instead of disarming it on the two biggest.
...
```bash
npx vitest run tests/unit/paired-axis-scale.test.ts
```
...
- **`/study/personas`'s compare view clips its fourth panel at four columns** — `scrollWidth` 1328 against a client width of 1066, inside an `overflow: hidden`. Measured by Task 7's implementer on the production build. It is pre-existing and the driver is the 297px cluster+archetype badge row, not anything this phase changed, but four pinned personas is a supported state and one of them is currently unreadable.
