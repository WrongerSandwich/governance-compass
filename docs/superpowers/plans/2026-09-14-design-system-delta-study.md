...
**One non-type site routed here from Task 7, because it is one decision across two files.** `PersonaModal.tsx:137` and `CompareView.tsx:45` each declare the mini budget strip's seven ministry fills as the six `--cluster-*` tokens plus a seventh spelled `var(--stone-400)` — a frozen ramp entry Task 14's section-wide guard bans, and bans correctly: the other six fills step by mode and this one cannot. Task 7 did **not** swap it, because unlike every other frozen-ramp site in the phase it has no 1:1 replacement. `--mark-primary` is Stone 600 in light, which is `--cluster-2`'s exact light value, and `--cluster-2` fills the *adjacent* segment — the swap merges segments 6 and 7 into one band. The same collision test rules out every other mode-stepping token `globals.css` declares: `--domain-economic` is `--mark-primary`'s value twin, `--domain-power` is `--cluster-5`'s light value, `--domain-society` and `--cluster-3` are the same two sage tones with the modes swapped, `--domain-world`'s dark tone lands beside `--cluster-1`'s, and `--text-label`'s dark tone is `--cluster-0`'s. A seventh distinct segment needs a seventh declared value: mint one in `globals.css` beside the cluster set, step it by mode like they do, and point both files at it. Doing it in this task keeps it one decision rather than two implementers each guessing; Task 14's guard is the forcing function either way.
