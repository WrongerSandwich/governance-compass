...
**As shipped.** The browser pass moved this task in eleven ways from the text above. Nothing in it was a formality: five of the eleven are defects the first eleven tasks could not have seen, because none of them ran a browser and four of the five are invisible to jsdom.

1. **The e2e suite was red before this task touched anything, and no task between 10 and 12 would have noticed.** `tests/e2e/quiz-flow.spec.ts:93` asserted `getByRole("heading", { name: "12-axis radar" })`; Task 10 retitled the section **Twelve-axis radar** and nothing pinned the old string outside Playwright. The unit suite cannot catch it — no unit test renders `ResultsView`'s section heads — so the break sat latent for two commits. Fixed by following the rename; the three assertions this phase's markup is coupled to (`Assessment results`, `Compare with someone`, `Paste their results link`) all passed **unmodified**. Final: **21 passed**.
...
10. **Both ratchets emptied in one commit, with the contrast measured in both modes and both guards mutation-checked afterwards.** The eight sub-AA occurrences went to three different tokens by role, and the ternary arms at `:153`/`:163` kept their *relationship* rather than merely clearing AA:
    ...
    | | before | after |
    |---|---|---|
    | tile labels, light | 2.99:1 | 5.95:1 |
    | tile labels, dark | 5.34:1 | 5.34:1 (identical tokens in dark) |
    | selected pole, light | 7.37:1 | 11.90:1 |
    | unselected pole, light | 3.28:1 | 6.52:1 |
    | selected pole, dark | 7.37:1 (a light beige box on a dark page) | 14.56:1 |
    | unselected pole, dark | 4.81:1 | 8.80:1 |
    | section headings, dark | **1.78:1** | 15.45:1 |
    | chosen scaled option, dark | 3.47:1 | 15.45:1 |

    The dark section-heading figure is the one that justifies the whole exercise: `text-stone-800` is fixed across modes, so three `<h2>`s all but vanished on the dark panel. After emptying, restoring a single `text-text-tertiary` reds the first guard and a single `text-stone-600` reds the second — verified, not assumed.

    One correction against this reconciliation's own first draft: the commit message and the guard comment both originally recorded that heading figure as **1.63:1**, which was reasoned from the token table rather than read off the page. Measured, it is **1.78:1**. Corrected in both places in a follow-up commit, and recorded here because "asserted without observing" is the failure mode this phase keeps repeating.
...
11. **The hydration mismatch was misdiagnosed in the plan, and the real cause changes the fix.** The plan carried it as a float-to-string precision difference. It is not a formatting difference — **the two engines compute different doubles.** `Math.sin`/`Math.cos` are not required by ECMA-262 to be correctly rounded, and Node and Chromium ship different ports of the routine, so `[data-radar-label]`'s `y` is `123.7231224733878` server-side and `123.72312247338783` client-side: 3e-14 SVG units apart, in the 14th significant digit. React reports it and explicitly **does not patch it up**, which is also why it is invisible from the production DOM — the DOM keeps the server value and reads byte-identical to the SSR HTML, and React's hydration warnings are development-only. It has to be seen in `next dev`. Six elements were affected, not one: both `[data-mini-ring]` polygons, `[data-radar-user]`, two `[data-radar-dot]` centres and the label.
    ...
    Fixed by rounding to nine decimal places inside `polarToCart`, which is the single chokepoint every coordinate on both charts passes through — a helper each emission site must remember to call is the exact duplication `radar-geometry.ts` was extracted to end. Nine places discards a 3e-14 disagreement with four orders of magnitude to spare and stays far inside the `1e-6` tolerances the pinned geometry assertions use; `radar-geometry.test.ts` and the Task 7-9 `MiniRadar` vertex assertions are **unchanged and green**. Verified after: zero console errors on `next dev`, where there had been exactly one.
