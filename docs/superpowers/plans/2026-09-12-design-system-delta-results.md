...
12. **Review's finding, and the one this task got wrong on its own terms: the two fixes only Task 12 could make were the two it shipped untested.** Every other change in this phase arrived with a guard. The hydration rounding and the compass sizing did not — reverting either left the suite green — and both are exactly the changes no earlier task could have written a test for, which is how they slipped. Closed in `ff9feef`: five string-level tests on the rounding, a `boundingBox()` assertion on the compass, and a title assertion covering the e2e break of note 1. All three were mutation-checked in both directions rather than merely added.
    ...
    **The whole Task 11 mutation table was re-run at this task's tip, not just the new guards: all sixteen rows still die, each to the test the table names.** Two notes for whoever repeats it. Rows 2 and 2b target two *different* `border-rule-strong` rules in `ArchetypeCard` — the distinctive branch's at `:135` and the primary branch's hooked one at `:172-175` — and a naive first-match replacement hits `:135` for both, which makes row 2 look as though its named test does not kill it. It does; the mutation was mis-aimed. And row 4b has to be applied as valid syntax: swapping the `className="…"` string for a template literal without closing it leaves vitest reporting no failures at all, which a harness that greps for failed-test lines will happily record as "survived". Both of those were caught here by re-checking rather than by the harness, which is the same lesson as the rest of this note.
